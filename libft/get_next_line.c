...
** @param t_node **god is an address of head pointer.
** @return Node's address to manage specific file descriptor.
*/

static t_node		*ft_find_fd_node(const int fd, t_node **god)
{
	t_node	*head;
	t_node	*new_node;
...
** @param t_node *h is address of file descriptor's node
** @param int command tells to use ft_strjoin or ft_strdup
** @return void
*/

static void			ft_refresh_store_buffer(t_node *h, int command)
{
	char	*new_node;
	char	*temp;
...
static int			ft_file_descriptor(t_node *h, char **line)
{
...
int					get_next_line(const int fd, char **line)
{
	...
	if (*god == 0)
	{
		*god = (t_node *)malloc(sizeof(t_node));
		if (*god != NULL)
		{
			(*god)->fd = fd;
			(*god)->blocker = 0;
